...
func main() {
	...
	downloadInstaller, _ := pkg.NewDownloadInstaller("https://releases.hashicorp.com/packer/{{ .Version }}/packer_{{ .Version }}_{{ .Os }}_{{ .Arch }}.zip", ctx)
	goBuildInstaller := pkg.NewGoBuildInstaller("https://github.com/hashicorp/packer.git", "packer", "", ctx)
	fallbackInstaller := pkg.NewFallbackInstaller(downloadInstaller, goBuildInstaller)
	var homeDir string
	var err error
	if homeDir = os.Getenv("PKENV_HOME_DIR"); homeDir != "" {
	...
	env := pkg.NewEnv(homeDir, "pkenv", "packer", fallbackInstaller)
	...
	var rootCmd = &cobra.Command{Use: "pkenv"}
